...
export class FrequencyChecker {
  ...
  checkLimit(key, limit, description) {
    ...
    if (!allowed) {
      console.log(
        ` Rate limit exceeded for ${description}: ${currentCount}/${limit} in last ${this.windowHours}h`
      );
    } else {
      console.log(
        ` Rate limit OK for ${description}: ${currentCount}/${limit} in last ${this.windowHours}h`
      );
    }
