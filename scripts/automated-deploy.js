...
class DeploymentManager {
  ...
  async verifyContractsOnBlockscout(contractAddress) {
    ...
    try {
      ...
      if (verificationResults.failed.length === 0) {
        ...
        console.warn(
          ` Some contracts failed verification: ${verificationResults.failed.join(', ')}`
        );
      }
    } catch (error) {
  ...
  async validateEnvironment() {
    ...
    try {
      ...
      if (response.result) {
        console.log(`  ✓ RPC connectivity: Chain ID ${Number.parseInt(response.result, 16)}`);
      } else {
  ...
  async runIntegrationTests() {
    ...
    try {
      ...
      const { stdout: result1 } = await execAsync(
        `curl -s "http://localhost:${config.port}/send/${testEvmAddress}"`
      );
      ...
      if (response1.result && response1.result.code === 0) {
        ...
        console.log(
          `   Cosmos TX hash: ${response1.result.transfers?.find((t) => t.type === 'cosmos_native')?.hash || 'N/A'}`
        );
      } else {
      ...
      const { stdout: result2 } = await execAsync(
        `curl -s "http://localhost:${config.port}/send/${testCosmosAddress}"`
      );
      ...
      const { stdout: result3 } = await execAsync(
        `curl -s "http://localhost:8088/send/${testEvmAddress}"`
      );
      ...
      const { stdout: result4 } = await execAsync(
        `curl -s "http://localhost:8088/send/${testCosmosAddress}"`
      );
      const response4 = JSON.parse(result4);

      if (response4.result && response4.result.message?.includes('sufficient balance')) {
        console.log(
          ' Test 4 passed: Cosmos rate limiting working (address has sufficient balance)'
        );
