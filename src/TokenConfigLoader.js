...
class TokenConfigLoader {
  ...
  loadConfig() {
    try {
      ...
      console.log(
        `✓ Loaded token configuration with ${this.tokensConfig.tokens.length} ERC20 tokens and ${this.tokensConfig.nativeTokens.length} native tokens`
      );
    } catch (error) {
  ...
  getNetworkConfig() {
    ...
    throw new Error(
      'Network configuration must be provided from config.js. tokens.json no longer contains network parameters.'
    );
  }
  ...
  getNativeTokens() {
    return this.tokensConfig.nativeTokens
      .filter((token) => token.faucet.enabled)
      .map((token) => ({
        denom: token.denom,
        name: token.name,
  ...
  getAllTokensForConfig() {
    ...
    nativeTokens.forEach((token) => {
      amounts.push({
        ...
        name: token.name,
        amount: token.amount,
        erc20_contract: token.evmWrapper?.enabled
          ? token.evmWrapper.wrapperContract
  ...
  getTokenBySymbol(symbol) {
    ...
    // Check native tokens
    const nativeToken = this.tokensConfig.nativeTokens.find(
      (token) => token.symbol === upperSymbol
    );
  ...
  updateFaucetAddresses(atomicMultiSendAddress, operatorAddress) {
    ...
    this.saveConfig();
    console.log(
      `✓ Updated faucet addresses - AtomicMultiSend: ${atomicMultiSendAddress}, Operator: ${operatorAddress}`
    );
  }
  ...
  updateNetworkConfig(chainId, cosmosChainId, networkName) {
    ...
    this.saveConfig();
    console.log(
      `✓ Updated network config - Chain ID: ${chainId}, Cosmos Chain ID: ${cosmosChainId}`
    );
  }
