...
export class ContractValidator {
  ...
  async validateTokenContract(tokenConfig) {
    ...
    try {
      ...
      try {
        ...
        const isNativeToken =
          tokenConfig.denom === 'uatom' &&
          tokenConfig.erc20_contract === '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE';
        if (!isNativeToken && symbol.toUpperCase() !== tokenConfig.denom.toUpperCase()) {
          ...
          console.log(
            `    Symbol mismatch: expected ${tokenConfig.denom.toUpperCase()}, got ${symbol}`
          );
          return result;
        ...
        if (!isNativeToken && Number(decimals) !== tokenConfig.decimals) {
          ...
          console.log(
            `    Native token wrapper - decimals validation skipped (expected ${tokenConfig.decimals}, got ${decimals})`
          );
        }
  ...
  async resolveValidationFailures() {
    ...
    try {
      ...
      for (const item of this.validationResults.needsManualInput) {
        ...
        console.log(
          `   Reason: ${this.validationResults[item.type === 'token' ? 'tokens' : 'atomicMultiSend'][item.denom || 0]?.reason || 'Unknown'}`
        );
        ...
        switch (action.trim()) {
          case '1': {
            const address = await question(
              `   Enter valid ${item.type === 'token' ? item.denom.toUpperCase() : 'AtomicMultiSend'} address: `
            );
          ...
          case '3':
            console.log(
              `     Skipping ${item.type === 'token' ? item.denom.toUpperCase() : 'AtomicMultiSend'} - may cause errors`
            );
            break;
          default:
  ...
  async updateConfigAddress(item, address) {
    try {
      console.log(
        `    Updating config with ${item.type === 'token' ? item.denom.toUpperCase() : 'AtomicMultiSend'}: ${address}`
      );
  ...
  async deployContract(item) {
    console.log(
      `    Deploying ${item.type === 'token' ? item.denom.toUpperCase() + ' token' : 'AtomicMultiSend'}...`
    );
    ...
    try {
      if (item.type === 'token') {
        ...
        const { stdout } = await exec(
          'REGISTRY_FILE=temp-token-registry.json node scripts/deploy-token-registry.js'
        );
        console.log(`    Token deployed successfully`);

        // Parse deployment output for address
        const report = JSON.parse(
          await fs.readFile('./deployments/token-deployment-report.json', 'utf8')
        );
